...
object HotelSessionManager {
     ...
     */
    fun markSessionPaired(context: Context, hotelId: String, roomNumber: String) {
    ...
    fun markSessionUnpaired(hotelId: String?, roomNumber: String?) {
        ...
        try {
            ...
            Log.w(TAG, "markSessionUnpaired failed for $path", e)
        }
    }
    ...
    fun performLogout(activity: Activity, reason: String) {
        ...
        AdminSession.clear()
        // Clears hotel/room prefs + hotel_tv_kiosk tenant cache + stops Watchdog.
        config.clearPairingSession()
